...
#
# Регламент пополнения (весь процесс обновления kb — docs/kb-refresh.md):
# - новый пост патча: секция "## Update <id> — YYYY-MM-DD" ВВЕРХУ файла
#   (новые выше), следующей строкой — URL поста; формат — по существующим
#   секциям
# - имена — в бэктиках; функции в "### XS: добавлено" — с сигнатурой
#   `ret name(params)` (так их находит since_update); константы — в
#   разделах "### Триггеры/константы", "### Modding…" и любых с "добавлено"
# - после пополнения: go run ./cmd/kbgen -diff (отчёт «что нового») →
#   go run ./cmd/kbgen (перегенерация данных)
