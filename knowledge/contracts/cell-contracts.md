...
- **Read-only в кодовых задачах**: менять CODEMANIFEST как попутное следствие
  ...
  (своя задача/PR). Расширение контракта новой задачей (напр. новый метод
  хендлера) правит CODEMANIFEST той же правкой и держит `goga contract`
  зелёным на каждом коммите.
- **Инварианты goga lint при расширении контракта** (выяснено в #118):
  каждый тип в `Imports.Types` обязан существовать в CODEMANIFEST
  ячейки-источника (`import_type_exists`), а каждая бэктик-ссылка в
  аннотациях обязана резолвиться в import/usage/entity
  (`annotation_links_exists`). Следствие: Go-сентинелы (например
  `ErrParseErrors`) не ссылаются бэктиками — их нет в манифестах; в
  аннотациях упоминают прозой («sentinel ErrParseErrors»).
