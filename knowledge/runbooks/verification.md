...
**Локальный `.corpus` не сверяется с пином** `corpus-sources.txt`: skip-логика
везде смотрит только на счётчик файлов (≥ 100). Устаревший сэмпл даёт ложные
падения корпусных тестов (и ложные пропуски) — перед выводами о регрессии
перекачать: `rm -rf .corpus && scripts/corpus-fetch.sh .corpus`
(#127).
