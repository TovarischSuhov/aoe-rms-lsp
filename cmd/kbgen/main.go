...
func main() {
	...
	if err := run(*refDir, *outDir); err != nil {
		slog.Error("kbgen failed", "error", err)
		os.Exit(1)
	}
}

// run regenerates all three data files.
func run(refDir string, outDir string) error {
	changelog, err := os.ReadFile(filepath.Join(refDir, "aoe2de-xs-rms-changelog.md"))
	if err != nil {
	...
	commands, err := kb.ExtractRmsCommands(filepath.Join(refDir, "zetnus-rms-guide.txt"))
	if err != nil {
	...
	if err := writeJSON(filepath.Join(outDir, "xs-functions.json"), functions); err != nil {
		return err
	}

	if err := writeJSON(filepath.Join(outDir, "xs-constants.json"), constants); err != nil {
		return err
	}
	...
	if err := writeJSON(filepath.Join(outDir, "rms-commands.json"), wire); err != nil {
		return err
	}

	slog.Info("knowledge base generated",
...
func argsToJSON(args []kb.CommandArg) []argJSON {
